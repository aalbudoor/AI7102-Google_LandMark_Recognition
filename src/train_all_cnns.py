...
# Logging setup — creates clean timestamped log files
logging.basicConfig(level=logging.INFO,format="%(asctime)s | %(levelname)s | %(message)s",datefmt="%H:%M:%S")
logger = logging.getLogger(__name__)

# Create a unique directory for this run using timestamp
...
# CNN Builder Function — builds a CNN with variable depth
def build_cnn(num_classes, depth, pretrained_path=None):
    """
    Builds a CNN with the specified depth.
    Structure: Conv2D → ReLU → MaxPool(2) repeated `depth` times.
    The number of channels doubles every two layers (up to 256).
    """
    layers = []
    in_channels = 3
    out_channels = 32
    ...
    # Classification head — flatten + two fully connected layers
    ...
    # Combine encoder + classifier into one sequential model
...
def pretrain_autoencoder(dl_train, device, save_path):
    ...
    # Simple pretraining loop for reconstruction
    ...
    # Save pretrained encoder weights for later use
...
# One-epoch training loop for the classifier CNN
...
# Validation loop — same as training but without gradients
...
# Main experiment routine — runs all depth experiments
def run_experiment(cfg):
    ...
    # Pretrain a single autoencoder to initialize all models
    encoder_path = RESULTS_DIR / "pretrained_encoder.pth"
    pretrain_autoencoder(dl_train, device, encoder_path)

    # Train CNNs with progressively increasing depth
    for depth in [3, 5, 7, 9, 11, 13]:
        ...
        # Training loop for several epochs
        ...
        # Report total time and best accuracy for this depth
